...
class FeedbackMatrix:
    ...
    def __init__(self,
        ...
        if feedback_matrix_type == FeedbackMatrixType.SCALAR_RANDOM:
            self.feedback_matrix = self.get_random_unitary_matrix()

        elif feedback_matrix_type == FeedbackMatrixType.SCALAR_HADAMARD:
            if np.log2(self.num_rows).is_integer():
                self.feedback_matrix = self.get_hadamard_matrix()
            else:
                raise RuntimeError(
    ...
    def get_random_unitary_matrix(self):
        # start with a random matrix and get its QR decomposition
        (Q, R) = qr(np.random.randn(self.num_rows, self.num_cols))
        return np.matmul(Q, np.diag(np.sign(np.diag(R))))

    def get_hadamard_matrix(self):
        hadamard_matrix = hadamard(self.num_rows) / np.sqrt(self.num_rows)
        if not self.is_unitary(hadamard_matrix):
    ...
    def populate_velvet_feedback_matrix(self,
        ...
        if np.log2(self.num_rows).is_integer():
            cur_dense_matrix = self.get_hadamard_matrix()[..., np.newaxis]
        else:
            cur_dense_matrix = self.get_random_unitary_matrix()[...,
                                                                np.newaxis]

        for k in range(num_mixing_stages):
            unitary_matrix = self.get_hadamard_matrix() if np.log2(
                self.num_rows).is_integer(
                ) else self.get_random_unitary_matrix()
    ...
    def process(self, input_data: NDArray) -> NDArray:
        ...
        if self.feedback_matrix_type in (FeedbackMatrixType.SCALAR_RANDOM,
                                         FeedbackMatrixType.SCALAR_HADAMARD):
            return self.feedback_matrix @ input_data
        else:
            temp_matrix = np.zeros(
                (self.num_rows, self.filter_order + self.frame_size - 1))
            for i in range(self.num_rows):
                for j in range(self.num_cols):
                    temp_matrix[i, :] += np.convolve(
                        self.feedback_matrix[i, j, :], input_data_copy[j, :])
